...
function sportLabel(league, sport) {
  const map = {
    "premier-league": "الإنجليزي", "la-liga": "الإسباني",
    "bundesliga": "البوندسليغا",   "serie-a": "السيريا آ",
    "ligue-1": "الليغ 1",          "champions-league": "أبطال أوروبا",
    "saudi-pro-league": "السعودي", "eredivisie": "الهولندي",
  };
          ...
          {featured && (
            <motion.div className="hero-featured" {...fadeUp(0)}>
              <Link href={`/articles/${featured.slug}/`} style={{ textDecoration: "none", display: "block", height: "100%" }}>
                <div className="card-hover hero-card-inner" style={{ position: "relative", borderRadius: "20px", overflow: "hidden", height: "100%", background: "#1a1f2e" }}>
                  <ArticleImage src={featured.image} imageUrl={featured.imageUrl} alt={featured.title} sport={featured.sport} league={featured.league} slug={featured.slug}
                    style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.93) 0%, rgba(0,0,0,0.3) 55%, transparent 100%)" }} />
                  {/* Live badge */}
                  <div style={{ position: "absolute", top: "16px", right: "16px", background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", padding: "5px 12px", borderRadius: "999px", border: "1px solid rgba(255,255,255,0.15)" }}>
              ...
              return (
                <motion.div key={article.slug} {...fadeUp(0.12 + i * 0.1)} style={{ flex: 1 }}>
                  <Link href={`/articles/${article.slug}/`} style={{ textDecoration: "none", display: "block", height: "100%" }}>
                    <div className="card-hover hero-sec-inner" style={{ position: "relative", borderRadius: "16px", overflow: "hidden", height: "100%", background: "#1a1f2e" }}>
                      <ArticleImage src={article.image} imageUrl={article.imageUrl} alt={article.title} sport={article.sport} league={article.league} slug={article.slug}
                        style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />
...
function SectionHeader({ title, href, delay = 0 }) {
  return (
    ...
    >
      <h2 style={{ margin: 0, fontSize: "19px", fontWeight: 800, color: "white", display: "flex", alignItems: "center", gap: "10px" }}>
        <span style={{ display: "block", width: "4px", height: "20px", borderRadius: "2px", background: "#3b82f6", flexShrink: 0 }} />
        {title}
      </h2>
      {href && (
        <Link href={href} style={{ color: "#3b82f6", fontSize: "12px", fontWeight: 700, textDecoration: "none" }}>
          عرض الكل ←
        </Link>
      )}
    </motion.div>
  ...
  return (
    <motion.div variants={staggerChild}>
      <Link href={`/articles/${article.slug}/`} style={{ textDecoration: "none", display: "block" }}>
        <div className="card-hover news-card-inner" style={{ background: "#161b27", borderRadius: "14px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
          <ArticleImage src={article.image} imageUrl={article.imageUrl} alt={article.title} sport={article.sport} league={article.league} slug={article.slug}
          ...
          <div style={{ padding: "13px 14px" }}>
            ...
            <h3 style={{ margin: "0 0 5px 0", fontSize: "14px", fontWeight: 800, color: "white", lineHeight: 1.55, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {article.title}
            </h3>
            <p style={{ margin: 0, fontSize: "12px", color: "rgba(255,255,255,0.45)", lineHeight: 1.7, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {article.description}
            </p>
          </div>
        </div>
...
function SidebarList({ articles, title }) {
  return (
    <div style={{ background: "#161b27", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden" }}>
      ...
      {articles.map((article, i) => {
        ...
        return (
          <Link key={article.slug} href={`/articles/${article.slug}/`} style={{ textDecoration: "none", display: "block" }}>
            <div className="sidebar-row" style={{ padding: "11px 16px", borderBottom: i < articles.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
              <div style={{ marginBottom: "4px" }}>
                <Badge accent={accent}>{sportLabel(article.league, article.sport)}</Badge>
              </div>
              <div style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.85)", lineHeight: 1.55, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                {article.title}
              </div>
              {article.publishedAt && (
                <div style={{ color: "rgba(255,255,255,0.28)", fontSize: "11px", marginTop: "4px" }}>
                  {formatDate(article.publishedAt)}
                </div>
              )}
            </div>
...
/* ── Sport row (horizontal scroll on mobile) ─────────── */
    ...
    >
      ...
      <div className="sport-row">
        {articles.map((article) => {
          ...
          return (
            <Link key={article.slug} href={`/articles/${article.slug}/`} style={{ textDecoration: "none", display: "block", flexShrink: 0 }} className="sport-row-card">
              <div className="card-hover" style={{ background: "#161b27", borderRadius: "14px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
                <ArticleImage src={article.image} imageUrl={article.imageUrl} alt={article.title} sport={article.sport} league={article.league} slug={article.slug}
                ...
                <div style={{ padding: "11px 12px" }}>
                  ...
                  <div style={{ fontSize: "13px", fontWeight: 800, color: "white", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {article.title}
                  </div>
                </div>
              </div>
